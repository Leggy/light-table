 ...
 *
 */
public final class PerlinImprovedNoise {

	/**
	 * Pseudo-random number from 0 to 255.
	 */
	private static final int p[] = new int[512], permutation[] = { 151, 160,
	...
	public static double noise(double x, double y, double z) {
		...
		int X = (int) Math.floor(x) & 255, Y = (int) Math.floor(y) & 255, Z = (int) Math
				.floor(z) & 255;
		/*
		 * Find relative x, y, z coordinates of point in cube.
		 */
		x -= Math.floor(x);
		y -= Math.floor(y);
		z -= Math.floor(z);
		...
		/*
		 * Has the coordinates of the 8 corners, and add blended results from 8
		 * corners of cube.
		 */
